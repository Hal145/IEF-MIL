...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def visHeatmap(self, scores, coords, vis_level=-1,
                   ...
                   square_size=200):
        if vis_level < 0:
            vis_level = 1 #self.wsi.get_best_level_for_downsample(32)
        vis_level=3
        downsample = self.level_downsamples[vis_level]
        scale = [1 / downsample[0], 1 / downsample[1]]
        print(scale)# Scaling from 0 to desired level
        ...
        if top_left is not None and bot_right is not None:
            ...
            region_size = self.level_dim[vis_level] #(5600, 3024)
            top_left = (0, 0)
        ...
        if blur:
            overlay = cv2.GaussianBlur(overlay, (35, 35), 0)
        ...
        # Draw fixed size squares around the top 3 scores
        top_indices = np.argsort(scores)[-3:]
        draw = ImageDraw.Draw(img)
        for i, idx in enumerate(top_indices):
            coord = coords[idx]
            draw.rectangle([coord[0], coord[1], coord[0] + square_size, coord[1] + square_size], outline="black", width=20)
            draw.text((coord[0] + 5, coord[1] + 5), str(i+1), fill="yellow", font=font)"""

        """"# Convert PIL Image to numpy array for processing
        img_np = np.array(img)

        # Create a function to handle the rectangle selection
        def onselect(eclick, erelease):
            nonlocal rect_coords
            rect_coords = (int(eclick.xdata), int(eclick.ydata),
                           int(erelease.xdata), int(erelease.ydata))
            plt.close()  # Close the interactive window to continue execution
        ...
        rect_selector = RectangleSelector(ax, onselect,
                                          ...
                                          spancoords='pixels',
                                          interactive=True)
        plt.title('Draw a rectangle and press Enter')
        plt.show()  # Wait for the user to complete the selection
        ...
        # Adjust region size for the selected rectangle
        region_size = (bot_right[0] - top_left[0], bot_right[1] - top_left[1])

        # Extract the selected area
        zoomed_img_np = img_np[top_left[1]:bot_right[1], top_left[0]:bot_right[0]]
        zoomed_img = Image.fromarray(zoomed_img_np)

        # Resize to zoom in
        zoom_factor = 1  # Adjust the zoom factor as needed
        zoomed_img_resized = zoomed_img.resize(
            (int(region_size[0] * zoom_factor), int(region_size[1] * zoom_factor)),
            Image.Resampling.BOX
        )

        zoomed_save_path ="/media/nfs/HL_dataset/HEATMAPS/zoomed--.png"
        print(save_path)
        if zoomed_save_path:
            zoomed_img.save(zoomed_save_path)

            # Compute and save heatmap for the zoomed area
            coords_in_rect = [(x, y) for (x, y) in coords if
                              top_left[0] <= x < bot_right[0] and top_left[1] <= y < bot_right[1]]
            scores_in_rect = [scores[i] for i in range(len(coords)) if
                              top_left[0] <= coords[i][0] < bot_right[0] and top_left[1] <= coords[i][1] < bot_right[1]]

            if len(coords_in_rect) == 0:
                print("No attention scores in the selected rectangle.")
                return None

            # Generate heatmap for the zoomed area
            zoomed_heatmap = np.full((int(region_size[1] * zoom_factor), int(region_size[0] * zoom_factor)), 0.0)
            zoomed_counter = np.full((int(region_size[1] * zoom_factor), int(region_size[0] * zoom_factor)), 0,
                                     dtype=np.uint16)

            for idx, (coord, score) in enumerate(zip(coords_in_rect, scores_in_rect)):
                if score >= thresh:
                    coord = (coord[0] - top_left[0], coord[1] - top_left[1])
                    coord_zoomed = (int(coord[0] * zoom_factor), int(coord[1] * zoom_factor))
                    zoomed_heatmap[coord_zoomed[1]:coord_zoomed[1] + patch_size[1] * zoom_factor,
                    coord_zoomed[0]:coord_zoomed[0] + patch_size[0] * zoom_factor] += score
                    zoomed_counter[coord_zoomed[1]:coord_zoomed[1] + patch_size[1] * zoom_factor,
                    coord_zoomed[0]:coord_zoomed[0] + patch_size[0] * zoom_factor] += 1

            zero_mask = zoomed_counter == 0
            if binarize:
                zoomed_heatmap[~zero_mask] = np.around(zoomed_heatmap[~zero_mask] / zoomed_counter[~zero_mask])
            else:
                zoomed_heatmap[~zero_mask] = zoomed_heatmap[~zero_mask] / zoomed_counter[~zero_mask]

            if blur:
                zoomed_heatmap = cv2.GaussianBlur(zoomed_heatmap, (35, 35), 0)

            zoomed_heatmap_img = Image.fromarray((zoomed_heatmap * 255).astype(np.uint8))
            zoomed_heatmap_save_path = save_path.replace('.csv', '_heatmap_zoomed.png') if save_path else None
            if zoomed_heatmap_save_path:
                zoomed_heatmap_img.save(zoomed_heatmap_save_path)"""
