...
class Wsi_Region(Dataset):
    ...
    def __init__(self, wsi_object, top_left=None, bot_right=None, level=0, 
        ...
        self.ref_downsample = [wsi_object.level_downsamples[level_] for level_ in level]
        # patch size in reference to level 0
        self.ref_size = [tuple((np.array(patch_size) * np.array(ref_downsample_)).astype(int)) for ref_downsample_ in self.ref_downsample]
        ...
        for cont_idx, contour in enumerate(wsi_object.contours_tissue): 
            ...
            cont_check_fn = [get_contour_check_fn(contour_fn, contour, ref_size[0], center_shift) for ref_size in self.ref_size]

            coord_results = []
            for i, _ in enumerate(level):
                coord_result, _ = wsi_object.process_contour(contour, wsi_object.holes_tissue[cont_idx], level[i], '',
                                           patch_size=patch_size[0], step_size=step_size[0],
                                           contour_fn=cont_check_fn[i],
                                           use_padding=True, top_left=top_left, bot_right=bot_right)
                coord_results.append(coord_result)
            if len(coord_results) > 0:
                new_coords = [coord['coords'] for coord in coord_results]
                filtered_coords.append(new_coords)
        
        coords=[[np.vstack(coord) for coord in coords] for coords in filtered_coords]
